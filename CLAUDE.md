...
4. shard 序号从 0 开始，4 位整数命名。无压缩 `0000.bin.def`，shard 级压缩 `0000.bin.zst.def`，segment 级压缩 `0000.zst.bin.def`。
...
- 压缩模式从 arcset 的 `metadata["compress"]` 读取，def 文件名中的扩展名仅用于标识。
