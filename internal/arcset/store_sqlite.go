package arcset

import (
	"context"
	"database/sql"
	"encoding/json"
	"path/filepath"
	"strings"
	"time"

	"github.com/kaichao/gopkg/errors"
	"github.com/sirupsen/logrus"
)

// SQLiteStore implements Store using SQLite.
type SQLiteStore struct {
	DB *sql.DB
}

func NewSQLiteStore(db *sql.DB) *SQLiteStore { return &SQLiteStore{DB: db} }

func (s *SQLiteStore) Create(ctx context.Context, a *Arcset) error {
	metadataJSON, _ := json.Marshal(a.Metadata)
	result, err := s.DB.ExecContext(ctx,
		`INSERT INTO t_arcset (name, label, metadata, status, current_path, last_check, comment)
		 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		a.Name, nullStr(a.Label), string(metadataJSON), nullStr(a.Status),
		nullStr(a.CurrentPath), nullTime(a.LastCheck), nullStr(a.Comment))
	if err != nil {
		return errors.WrapE(err, "create arcset", "name", a.Name)
	}
	id, _ := result.LastInsertId()
	a.ID = int(id)
	return nil
}

func nullStr(s string) any  { if s == "" { return nil }; return s }
func nullTime(t time.Time) any { if t.IsZero() { return nil }; return t }

func (s *SQLiteStore) FindByID(ctx context.Context, id int) (*Arcset, error) {
	query := `SELECT id, name, label, metadata, status, current_path, last_check, comment
	           FROM t_arcset WHERE id = ?`
	return s.scanOne(s.DB.QueryRowContext(ctx, query, id))
}

func (s *SQLiteStore) FindByName(ctx context.Context, name string) (*Arcset, error) {
	query := `SELECT id, name, label, metadata, status, current_path, last_check, comment
	           FROM t_arcset WHERE name = ?`
	return s.scanOne(s.DB.QueryRowContext(ctx, query, name))
}

func (s *SQLiteStore) Find(ctx context.Context, filter Filter) ([]*Arcset, error) {
	query := `SELECT id, name, label, metadata, status, current_path, last_check, comment
	           FROM t_arcset WHERE 1=1`
	var args []any

	if filter.Status != nil {
		query += " AND status = ?"
		args = append(args, *filter.Status)
	}
	query += " ORDER BY name"

	rows, err := s.DB.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, errors.WrapE(err, "find arcsets")
	}
	defer rows.Close()

	var arcsets []*Arcset
	for rows.Next() {
		a, err := s.scanRow(rows)
		if err != nil {
			return nil, err
		}
		arcsets = append(arcsets, a)
	}
	return arcsets, rows.Err()
}

func (s *SQLiteStore) Update(ctx context.Context, name string, u Update) error {
	query := "UPDATE t_arcset SET "
	var sets []string
	var args []any

	if u.Status != nil {
		sets = append(sets, "status = ?")
		args = append(args, *u.Status)
	}
	if u.Label != nil {
		sets = append(sets, "label = ?")
		args = append(args, *u.Label)
	}
	if u.Comment != nil {
		sets = append(sets, "comment = ?")
		args = append(args, *u.Comment)
	}
	if u.LastCheck != nil {
		sets = append(sets, "last_check = ?")
		args = append(args, *u.LastCheck)
	}
	if u.Metadata != nil {
		metadataJSON, _ := json.Marshal(u.Metadata)
		sets = append(sets, "metadata = ?")
		args = append(args, string(metadataJSON))
	}

	if len(sets) == 0 {
		return nil
	}
	query += strings.Join(sets, ", ") + " WHERE name = ?"
	args = append(args, name)

	_, err := s.DB.ExecContext(ctx, query, args...)
	return errors.WrapE(err, "update arcset", "name", name)
}

func (s *SQLiteStore) AddDataset(ctx context.Context, arcsetID, datasetID int) error {
	_, err := s.DB.ExecContext(ctx,
		`INSERT INTO r_arcset_dataset (arcset, dataset) VALUES (?, ?)`,
		arcsetID, datasetID)
	return errors.WrapE(err, "add dataset to arcset", "arcset_id", arcsetID, "dataset_id", datasetID)
}

func (s *SQLiteStore) ListDatasetRefs(ctx context.Context, arcsetID int) ([]DatasetRef, error) {
	query := `SELECT d.id, d.name
	           FROM t_dataset d
	           JOIN r_arcset_dataset r ON r.dataset = d.id
	           WHERE r.arcset = ?
	           ORDER BY d.name`
	rows, err := s.DB.QueryContext(ctx, query, arcsetID)
	if err != nil {
		return nil, errors.WrapE(err, "list dataset refs", "arcset_id", arcsetID)
	}
	defer rows.Close()

	var refs []DatasetRef
	for rows.Next() {
		var ref DatasetRef
		if err := rows.Scan(&ref.ID, &ref.Name); err != nil {
			return nil, errors.WrapE(err, "scan dataset ref")
		}
		refs = append(refs, ref)
	}
	return refs, rows.Err()
}

func (s *SQLiteStore) ListArcsetFiles(ctx context.Context, arcsetID int) ([]FileRow, error) {
	query := `SELECT f.id, f.file_path, COALESCE(f.file_size, 0), COALESCE(f.checksum, ''), f.dataset
	           FROM t_file f
	           JOIN r_arcset_dataset r ON r.dataset = f.dataset
	           WHERE r.arcset = ?
	           ORDER BY f.dataset, f.file_path`
	rows, err := s.DB.QueryContext(ctx, query, arcsetID)
	if err != nil {
		return nil, errors.WrapE(err, "list arcset files", "arcset_id", arcsetID)
	}
	defer rows.Close()

	var files []FileRow
	for rows.Next() {
		var f FileRow
		if err := rows.Scan(&f.ID, &f.FilePath, &f.FileSize, &f.Checksum, &f.DatasetID); err != nil {
			return nil, errors.WrapE(err, "scan file row")
		}
		files = append(files, f)
	}
	return files, rows.Err()
}

func (s *SQLiteStore) scanOne(row *sql.Row) (*Arcset, error) {
	a := &Arcset{}
	var (
		label, status, currentPath, comment sql.NullString
		lastCheck                            sql.NullTime
		metadata                             []byte
	)
	err := row.Scan(&a.ID, &a.Name, &label, &metadata, &status,
		&currentPath, &lastCheck, &comment)
	if err == sql.ErrNoRows {
		return nil, errors.E("arcset not found")
	}
	if err != nil {
		return nil, errors.WrapE(err, "scan arcset row")
	}
	a.Label = label.String
	a.Status = status.String
	a.CurrentPath = currentPath.String
	a.Comment = comment.String
	if lastCheck.Valid {
		a.LastCheck = lastCheck.Time
	}
	a.Metadata = make(map[string]any)
	if len(metadata) > 0 {
		if err := json.Unmarshal(metadata, &a.Metadata); err != nil {
			logrus.Errorf("unmarshal arcset metadata failed: %v", err)
		}
	}
	return a, nil
}

func (s *SQLiteStore) scanRow(rows *sql.Rows) (*Arcset, error) {
	a := &Arcset{}
	var (
		label, status, currentPath, comment sql.NullString
		lastCheck                            sql.NullTime
		metadata                             []byte
	)
	if err := rows.Scan(&a.ID, &a.Name, &label, &metadata, &status,
		&currentPath, &lastCheck, &comment); err != nil {
		return nil, errors.WrapE(err, "scan arcset row")
	}
	a.Label = label.String
	a.Status = status.String
	a.CurrentPath = currentPath.String
	a.Comment = comment.String
	if lastCheck.Valid {
		a.LastCheck = lastCheck.Time
	}
	a.Metadata = make(map[string]any)
	if len(metadata) > 0 {
		if err := json.Unmarshal(metadata, &a.Metadata); err != nil {
			logrus.Errorf("unmarshal arcset metadata failed: %v", err)
		}
	}
	return a, nil
}

func (s *SQLiteStore) FindFileMap(ctx context.Context, arcsetID int) (map[string]FileMapEntry, error) {
	query := `SELECT f.file_path, d.current_path, f.id
	           FROM t_file f
	           JOIN t_dataset d ON d.id = f.dataset
	           JOIN r_arcset_dataset r ON r.dataset = d.id
	           WHERE r.arcset = ?`
	rows, err := s.DB.QueryContext(ctx, query, arcsetID)
	if err != nil {
		return nil, errors.WrapE(err, "find file map", "arcset_id", arcsetID)
	}
	defer rows.Close()

	m := make(map[string]FileMapEntry)
	for rows.Next() {
		var relPath, currentPath string
		var fileID int
		if err := rows.Scan(&relPath, &currentPath, &fileID); err != nil {
			return nil, errors.WrapE(err, "scan file map row")
		}
		m[relPath] = FileMapEntry{
			AbsPath: filepath.Join(currentPath, relPath),
			FileID:  fileID,
		}
	}
	return m, rows.Err()
}

func (s *SQLiteStore) FindFileAbsPath(ctx context.Context, fileID int) (string, error) {
	var relPath, currentPath string
	err := s.DB.QueryRowContext(ctx,
		`SELECT f.file_path, d.current_path
		 FROM t_file f
		 JOIN t_dataset d ON d.id = f.dataset
		 WHERE f.id = ?`, fileID).Scan(&relPath, &currentPath)
	if err == sql.ErrNoRows {
		return "", errors.E("file not found", "file_id", fileID)
	}
	if err != nil {
		return "", errors.WrapE(err, "find file abs path")
	}
	return filepath.Join(currentPath, relPath), nil
}

func (s *SQLiteStore) FindDatasetPath(ctx context.Context, datasetID int) (string, error) {
	var path string
	err := s.DB.QueryRowContext(ctx,
		`SELECT current_path FROM t_dataset WHERE id = ?`, datasetID).Scan(&path)
	if err == sql.ErrNoRows {
		return "", errors.E("dataset not found", "dataset_id", datasetID)
	}
	return path, errors.WrapE(err, "find dataset path")
}
